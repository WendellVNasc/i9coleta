...
const CardCacambaShop = ( { item, action = () => {} } : CardCacambaShopInterface ) => {
    ...
    return (
        <div className="card-cacamba" onClick={() => navigate(`/painel/pedircacamba/cacamba/${item?.ID}`)}>
            <div className='card-cacamba-div'>
                <Typography onClick={() => navigate(`/painel/pedircacamba/fornecedor/${item?.CREDENTIAL_ID}`)} className='card-cacamba-title'><Avatar src={item?.PROVIDER_PHOTO} className='card-cacamba-title-avt'/>{String(item?.PROVIDER_NAME).toLocaleUpperCase()}</Typography>
                <div className='card-cacamba-text-row'>
                    ...
                    <Typography className="card-cacamba-subtitle"><span className='star'><IoStar style={{marginLeft: '0.2em', marginRight: '0.2em'}} /> 0 </span> • 0 Km</Typography>
                </div>
