...
const AguardandoRetiradaList = ({ type, path, permission } : PageDefaultProps ) => {
    ...
    const column = [
        { title: 'Data Pedido Retirada', dataIndex: 'DATE_RETIRADA_FORMAT', table: 'order_location_product.DATE_RETIRADA', width: '180px', sorter: true, align: 'center', render: null },
        ...
        { title: 'Data Retirada', dataIndex: 'DATE_RETIRADA_DELIVERY', table: 'order_location_product.DATE_RETIRADA_DELIVERY', width: '180px', sorter: true, align: 'center', render: (item:any) => (
            <Row justify={'center'} style={{width: '100%'}}>
                ...
                { item.DATE_RETIRADA_DELIVERY ? <Tag color="red">{item.VEHICLE_RETIRADA_PLATE} - {item.VEHICLE_RETIRADA_TYPE_NAME}</Tag> : null }
            </Row>
        ) },
    ...
    const onSend = (values:any) => {
        ...
        POST_API('/order_location/retirada_delivery.php', { token: getToken(), master: JSON.stringify(values) }).then(rs => rs.json()).then(res => {
            if (res.return) {
                form.resetFields()
                setProduct([])
                setAction(!action)
                ...
                <Col xs={24} md={8}>
                    <CardItem title={`Agendar retirada | ${product.length} selecionado(s)`}>
                        <Form layout="vertical" form={form} onFinish={onSend}>
                            <Form.Item label="Data retirada" name="DATE_RETIRADA_DELIVERY" rules={[{required: true, message: 'Campo obrigatório!'}]}>
                                <Input type="date" min={new Date().toISOString().slice(0, 10)} />
                            </Form.Item>
                            <Form.Item label="Motorista" name="DRIVER_RETIRADA_ID" rules={[{required: true, message: 'Campo obrigatório!'}]}>
                                <SelectSearch placeholder="Nome - CNH" effect={driver} value={form.getFieldValue('DRIVER_RETIRADA_ID')} url="/driver/select.php" change={(v:any) => form.setFieldValue('DRIVER_RETIRADA_ID', v?.value)} />
                            </Form.Item>
                            <Form.Item label="Veiculo" name="VEHICLE_RETIRADA_ID" rules={[{required: true, message: 'Campo obrigatório!'}]}>
                                <SelectSearch placeholder="Placa - Tipo" effect={vehicle} value={form.getFieldValue('VEHICLE_RETIRADA_ID')} url="/vehicle/select.php" change={(v:any) => form.setFieldValue('VEHICLE_RETIRADA_ID', v?.value)} />
                            </Form.Item>
