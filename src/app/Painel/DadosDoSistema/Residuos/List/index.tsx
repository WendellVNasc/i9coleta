...
const ResiduosList = ({ type, path, permission } : PageDefaultProps ) => {
    ...
    const column = [
        { title: 'Nome', dataIndex: 'NAME', table: 'reside.NAME', width: '300px', sorter: true, align: 'center', render: null },
        { title: 'Descrição', dataIndex: 'DESC', table: 'reside.DESC', width: 'auto', sorter: true, align: 'left', render: null },
    ...
    return (
        <PageDefault valid={`${permission}.${type}`} items={[
            { title: <Link to={ type === 'list' ? '#' : '..' }>Grupo de Resíduo</Link> },
