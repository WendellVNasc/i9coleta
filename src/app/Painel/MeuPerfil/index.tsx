...
const MeuPerfil = () => {
  ...
  const onChangePic = (value: any) => {
    if (value.file.response?.url) {
        POST_API(`/user`, { photo: value.file.response?.url }, user?.id)
        .then((rs) => {
            if (rs.ok) {
                return rs.json();
            } else {
  ...
  return (
    <PageDefault valid={true} items={[{ title: "Meu Perfil" }]}>
      {user ? (
            ...
            <CardItem>
              <Form layout="vertical" form={form} onFinish={onSend}>
                <Row gutter={[8, 8]}>
                  ...
                  <Col xs={12} md={5}>
                    <Form.Item name="phone" label="Telefone Principal">
